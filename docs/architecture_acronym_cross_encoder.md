...
Trước khi đi sâu vào nền tảng toán học của Cross-Encoder, chúng ta cần mổ xẻ tập dữ liệu `acrDrAid` để thấu hiểu tại sao bài toán Acronym Disambiguation lại là một trong những rào cản lớn nhất của Xử lý Ngôn ngữ Tự nhiên Y tế. Ba khía cạnh dưới đây sẽ trả lời cho câu hỏi: *"Tại sao phải là Cross-Encoder?"*

### 1.1 Mức độ Đa nghĩa của Từ viết tắt (Semantic Ambiguity)
...
Tuy nhiên, phân tích kiểm chứng độ dài số từ (word count) cho thấy một bức tranh khả quan:
...
### 2.1 Tại sao Cross-Encoder vượt trội hơn Multi-class Classification?
...
## 3. Tầng 1: Tiền xử lý Dữ liệu (Data Preprocessing Layer)
...
### 3.3 Kỹ thuật Hard Negative Mining
...
### 3.4 Định dạng Đầu vào cho Tokenizer (Sentence-Pair Encoding)
...
### 3.5 Collate Function — Xử lý Batch có Kích thước Bất đồng nhất
...
### 4.2 Forward Pass — Luồng Tính toán Chi tiết
...
### 4.3 Hàm Mất mát: BCEWithLogitsLoss
...
### 4.4 Chiến lược Training nâng cao
...
## 5. Tầng 3: Chiến lược Suy luận (Inference Strategy)

### 5.1 Luồng Inference End-to-End
...
### 5.2 Batch Inference — Tối ưu Latency
...
### 5.3 Chọn Đáp án Cuối cùng & Confidence Score
...
## 6. Kết quả Đánh giá & Phân tích

### 6.1 Benchmark trên Test Set (1130 samples)
...
### 7.2 Tích hợp vào FastAPI Pipeline
